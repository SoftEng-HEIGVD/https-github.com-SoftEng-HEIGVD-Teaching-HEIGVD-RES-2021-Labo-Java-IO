...
public class Application implements IApplication {
  ...
  void storeQuote(Quote quote, String filename)  {
    String content = WORKSPACE_DIRECTORY;

    //ajout des sous dossier
    for (String tag : quote.getTags()) {
      content += "/" + tag;
    }
    ...
    try{
    FileUtils.writeStringToFile(new File(content),quote.getQuote()); // Je ne comprends par le flag "deprecated"
    } catch (IOException e) {
    e.printStackTrace();
  }
